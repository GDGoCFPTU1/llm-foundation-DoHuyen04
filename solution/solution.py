...
def compare_models(prompt: str) -> dict:
    ...
    """
    gpt4o_response, gpt4o_latency, gpt4o_usage = call_openai(prompt, OPENAI_MODEL)
    gpt4o_mini_response, gpt4o_mini_latency, gpt4o_mini_usage = call_openai(prompt, OPENAI_MINI_MODEL)
    gemini_response, gemini_latency, gemini_usage = call_gemini(prompt)

    def calc_cost(usage: dict, model: str) -> float:
        return (
            usage["input_tokens"] * PRICING_1M_TOKENS[model]["input"]
            + usage["output_tokens"] * PRICING_1M_TOKENS[model]["output"]
        ) / 1_000_000
    ...
    return {
        "gpt4o": {
            ...
            "cost": calc_cost(gpt4o_usage, "gpt-4o"),
        ...
        "gpt4o_mini": {
            ...
            "cost": calc_cost(gpt4o_mini_usage, "gpt-4o-mini"),
        ...
        "gemini_flash": {
            ...
            "cost": calc_cost(gemini_usage, "gemini-2.5-flash"),
...
def streaming_chatbot() -> None:
    ...
    print("Gemini 2.5 Chatbot (type 'quit' or 'exit' to exit)")
    print("-" * 50)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        ...
        for model_key in ["gpt4o", "gpt4o_mini", "gemini_flash"]:
            ...
            response = stats["response"][:50] + "..." if len(stats["response"]) > 50 else stats["response"]
            latency = f"{stats['latency']:.2f}s"
            tokens = f"{stats['input_tokens']}/{stats['output_tokens']}"
            cost = f"${stats['cost']:.6f}"
            lines.append(f"| {prompt} | {model_display_names[model_key]} | {response} | {latency} | {tokens} | {cost} |")
    return "\n".join(lines)
